...
import CartList from './PendingOrdersList';
...
export default class PendingOrders extends Component{
  ...
  componentWillUnmount() {
	...
	async ordersRetrieveHandler(){
		...
		let res = await axios.get("http://100.25.15.160/orders/ordersforcollection/active/order/",{
	...
	mapOrderData(){
		return this.state.data.map((t,key)=>{return(<CartList fun={this.reloadFun} data={t} key={key}/>)})
	}
			...
			return(
				<View style={styles.container}>
					...
					<View style={{flex:1,alignItems:"center",justifyContent:"center"}}> 
					 <Text style={{color:"#17baa1",fontSize:18}}>No Pending Order</Text>
